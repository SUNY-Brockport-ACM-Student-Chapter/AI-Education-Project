...
class ExamRepository:
    ...
    def get_exams_for_teacher(self, teacher_id: int):
        ...
        for course in courses:
            exams.extend(self.session.query(Exam).filter(Exam.course_id == course.course_id).all())
        return exams
    ...
    def get_student_exam_submission_stage(self, exam_id: int, student_id: int):
        ...
        first_exam_question = self.session.query(Question).filter(Question.exam_id == exam_id).first()
        if not first_exam_question:
            raise ValueError("Exam question not found")
        student_answer = self.session.query(StudentAnswer).filter(StudentAnswer.student_id == student_id, StudentAnswer.question_id == first_exam_question.question_id).first()
